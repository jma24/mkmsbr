; fat32_pbr_ntldr.asm — bootrec FAT32 PBR, Windows 2000/XP/2003 variant.
;
; Loaded by the MBR at 0000:7C00 in real mode, with DL = boot drive.
;
; Same algorithm as fat32_pbr_bootmgr.asm but searches the FAT32 root
; for "NTLDR      " (the 11-byte 8.3 form of NTLDR) instead of BOOTMGR.
; NTLDR is the XP-era boot loader; for Vista+ install media use
; fat32_pbr_bootmgr.asm.
;
; Layout:
...
.skip:
    ...
    cmp eax, 0x0FFFFFF8
    jb .dir_cluster
    ; Walked off end of root without finding NTLDR; stack balanced here.
.nf:
    mov al, '1'                       ; error code 1: NTLDR not in root
    jmp die
...
.found:
    ...
    ; Load NTLDR cluster chain. ES advances 0x20 per sector (= 512 bytes
...
;   '1' = NTLDR not found in FAT32 root directory
